		...
		if($('.gooid'+val).length > 0){
			...
			if($.inArray(po, ponum)!=-1){
				...
				$('.gooid'+val).each(function(){
					...
					$('.tfoot2').hide();
					$("#MaintainanceTable tbody").append('<tr id="itemrow'+us+'"><td>'+val+'</td><td>'+item+'('+alias+')</td><td>'+uom+'</td><td>'+qty+'</td><td>'+rate+'</td><td>'+disamt+' ('+disper+'%)</td><td class="ltd">'+amt+'</td></tr>');
					$('.tfoot').show();
				});

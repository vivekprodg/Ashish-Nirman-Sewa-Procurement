...
function url(){
  var name = $('#vehi_type').val();
  $('#url').val(name);
  var mat_url = $('#url').val().toLowerCase();
  var result = mat_url.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '');
  $('#url').val(result);
}
$('#vehi_type').on('keyup', function(){
	url();
});
$('#VehicleTypeForm').on('submit', function(){
	var error = 0;
	$('#spinner1').show();
	var name = $('#vehi_type').val();
	var url = $('#url').val();
	...
	if(error==0){
		document.VehicleTypeForm.submit();
	}else{
		$('#spinner1').hide();
	}
...
$('#VehicleTypeEditForm').on('submit', function(){
	var error = 0;
	$('#spinner2').show();
	var name = $('#editname').val();
	var url = $('#editurl').val();
	...
	if(error==0){
		document.VehicleTypeEditForm.submit();
	}else{
		$('#spinner2').hide();
	}
	event.preventDefault();

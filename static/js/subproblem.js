...
function url(){
  var name = $('#subproblem').val();
  $('#url').val(name);
  var mat_url = $('#url').val().toLowerCase();
  var result = mat_url.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '');
  $('#url').val(result);
}
$('#subproblem').on('keyup', function(){
	url();
});
$('#problemval').on('change', function(){
	var idstr = $("#problemval option:selected").val();
	var prob = $('#prob'+idstr).val();
	$('#problem_name').val(prob);
});
$('#ProblemForm').on('submit', function(){
	...
	var name = $('#subproblem').val();
	var url = $('#url').val();
	var prob = $('#problemval option:selected').val();
	if(name==''){
		error = 1;
		$('#subproblem').addClass('errorcolor');
	}
	if(url==''){
		error = 1;
		$('#subproblem').addClass('errorcolor');
	...
	if(prob==''){
		error = 1;
		$('#problemval').addClass('errorcolor');
	}
	if(error==0){
...
$('.sup_edit').click(function(){
	...
	var pname = $('#prob_name'+idstr).val();
	var purl = $('#prob_url'+idstr).val();
	$('#luid').val(idstr);
	$('#editname').val(name);
	$('#default').val(name);
	$('#editurl').val(url);
